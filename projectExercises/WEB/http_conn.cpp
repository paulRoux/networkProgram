...
/*网站根目录*/
const char* doc_root = "/war/www/html";
...
/*当得到一个完整且正确的HTTP请求时，则分析目标文件的属性。
 * 如果目标文件存在、对所有用户可读且不是目录，
...
bool http_conn::write()
{
	...
	while (1) {
		...
		if (temp <= -1) {
			/*如果TCP写缓冲区没有空间，则等待下一轮的EPOLLOUT事件。
			 * 虽然在此期间服务器无法立即接收到同一客户的下一个请求，但是可以保证连接的完整性*/
			if (errno == EAGAIN) {
...
bool http_conn::process_write(HTTP_CODE ret)
{
	switch (ret) {
		case INTERNAK_ERROR: {
			...
			add_headers((int)strlen(error_500_form));
		...
		case BAD_REQUEST: {
			...
			add_headers((int)strlen(error_400_form));
		...
		case NO_RESOURCE: {
			...
			add_headers((int)strlen(error_404_form));
		...
		case FILE_REQUEST: {
			...
			if (m_file_stat.st_size != 0) {
				...
				m_iv[0].iov_len = (size_t)m_write_idx;
